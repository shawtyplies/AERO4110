...
k = 1/(np.pi*AR*e)
k_g = k*((33*((4.62/32)**1.5))/(1+(33*((4.62/32)**1.5))))
print(f"{k}")
print(f"{k_g}")
...
v_to = 1.2*V_stall_f # light aircraft usually takeoff w no flaps (Raymer)
...
print(f"min power speed = {V_mp*1.944} knots")
...
L_climb = W*np.cos(gamma)
LDR_climb = L_climb/D_climb
print(f"Lift to drag at climb = {LDR_climb}")

# Ground roll (take-off)
mu = 0.5 # rolling resistance of wet grass
a = g*((((T_to/W)-mu) + ((rho/(2*W/S)))*(-C_D_0-(k*C_L_max_nf**2)+(mu*C_L_max_nf))*v_to**2))
print(f"ground acceleration = {a}m/s^2")
K_T = (T_to/W) - mu
print(f"K_T = {K_T}")
K_A = rho/(2*W/S)*((mu*C_L_g)-C_D_0-(k_g*(C_L_g**2)))
print(f"K_A = {K_A}")
v_i = 0
...
print(f"Transition speed = {v_tran*1.944} knots")
V_mp = np.sqrt(((2*W)/(rho*S))*np.sqrt(k/(3*(C_D_0+0.02)))) # m/s
print(f"Velocity at min poer = {V_mp} m/s")
...
R_tran = (v_tran**2) / (0.2*9.81)
print(f"Radius of transition arc = {R_tran*3.281} ft")
S_tran = R_tran*np.sin(gamma)
h_tran = R_tran*(1-np.cos(gamma))
print(f"Altitude cleared at transition = {h_tran*3.281} ft")
h_obs = 50/3.281 # m
#S_tran = np.sqrt(R_tran**2-((R_tran-h_obs)**2)) # the obstacle clearance is cleared after transition so this eq is used
...
v_TD = 1.15*V_stall_f
print(f"Touchdown speed = {v_TD*1.944} knots")
v_f = 1.23*V_stall_f
print(f"Flare speed = {v_f*1.944} knots")
R_f = (v_f**2) / (0.2*g)
print(f"Radius of flare = {R_f*3.281} ft")
h_f = R_f*(1-np.cos(gamma_a))
print(f"Flare height = {h_f*3.281} ft")
...
S_fr = 1.5*v_TD
print(f"Free roll distance = {S_fr} m")
print(f"Free roll distance = {S_fr*3.281} ft")
...
# print(f"K_A = {K_A}")
#S_G_a = (1/(2*g*K_A_a))*np.log((K_T_a/(K_T_a+(K_A_a*v_TD**2)))) # m
S_G_a = (v_TD**2) / (2*mu*9.81)
# S_G_a = (v_TD**2) / (2*mu*(v_TD/np.sqrt(2)))
print(f"Ground roll distance (landing) = {S_G_a} m")
print(f"Ground roll distance (landing) = {S_G_a*3.281} ft")

# Total landing
S_l = S_a + S_f + S_G_a
print(f"Total landing distance = {S_l} m")
print(f"Total landing distance = {S_l*3.281} ft")