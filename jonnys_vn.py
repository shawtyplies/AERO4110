...
v_stall = 37.4  # Stall speed (knots)
v_cruise = 102  # Cruise speed (knots)
v_dive = 146  # Maximum dive speed (knots)
W_max = 1990  # Maximum take off weight (lb)
...
n_maneuver_neg = np.piecewise(
    V,
    [V < v_cruise, V >= v_cruise],
    [lambda v: np.clip(-(v / v_stall) ** 3, n_min, 0), np.nan]
)
...
plt.plot([v_cruise, v_dive], [neg_cruise_y, neg_dive_y], 'red', linestyle='--')
...
plt.plot([v_cruise, v_dive], [pos_cruise_y, pos_dive_y], 'red', linestyle='--')

# Define the load factor at cruise speed for the negative maneuver envelope
load_factor_at_cruise = np.clip(-(v_cruise / v_stall) ** 3, n_min, 0)

# Create a line segment from (v_cruise, load_factor_at_cruise) to (v_dive, 0)
V_extension = [v_cruise, v_dive]
n_extension = [load_factor_at_cruise, 0]

# Plot the extension line
plt.plot(V_extension, n_extension, color='blue', linestyle='-')
